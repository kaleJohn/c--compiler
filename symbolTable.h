...
public:
    ...
    //custom functions
    void *getScopeStart();                               //returns a pointer associated with the symbol that started the current scope
    void setScopeStart(void *start);                    //sets a pointer associated with the symbol that started the current scope

    void *getCurrentFunction();                          //returns a pointer associated with the symbol that names the current function
    void setCurrentFunction(void *funct);               //sets a pointer associated with the symbol that names the current function
    bool getDebug();                                    //return the debug status
    void addToLoopNum(int val);                         //changes the loop number (usually by + or - 1)
    int getLoopNum();                                   //returns the loop number
};
