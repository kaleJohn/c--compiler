...
{
    char *space;//holds orig message
    char *strs[100];//holds each split message
    int numstrs;
    char* lastToken = tokenStore.at(tokNum-1);
    std::fill(strs, strs+100, nullptr);
    //int lineno = yylineno;
    ...
    // print components
    if(strs[3] == nullptr) {
        printf("ERROR(%d): Syntax error, unexpected %s", yylineno, lastToken);
    } else {
